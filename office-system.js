(() => {
  const VERSION = 'v12.4.3-negotiation-financial-engine';
  ...
  function createWonDealFromNegotiation(){
    ...
    const record={date:today(), invoiceNo:n.quotationNo, negotiationNo:n.no, prospect:n.customer, amount:Number(n.finalAmount||n.amount||0), method:'Pending Payment', ref:'WON-'+n.no};
    db.payments.unshift(record);
    n.wonDealCreated=true;
    n.notes = n.notes || [];
    n.notes.unshift({date:today(), text:`Won Deal created daripada ${n.no} dengan nilai ${money(record.amount)}.`});
  ...
  function renderAll(){
    ...
    const pt=$('#paymentsTbody'); if(pt) pt.innerHTML=db.payments.map(p=>`<tr><td>${p.date}</td><td>${p.invoiceNo}</td><td>${esc(p.prospect||'-')}</td><td>${money(p.amount)}</td><td>${p.method}</td><td>${p.ref||'-'}</td></tr>`).join('') || '<tr><td colspan="6" class="muted">No won deal yet.</td></tr>';
