(() => {
  const VERSION = 'v12.6-project-intake-engine';
  ...
  const seed = {
    ...
    quotations: [],
    negotiations: [],
    invoices: [], payments: [], projects: [], intakes: [], documents: []
  };
  ...
  function projectsHTML(){return `<div class="section-head office-section"><div><div class="eyebrow">Project Intake</div><h1>Won Projects</h1><p class="muted">Deal yang sudah menang, daftar sebagai projek dan lengkapkan intake sebelum dihantar ke Office RH.</p></div></div><div class="editor-card"><h2>Create Won Project</h2><div class="office-form-grid"><div><label>Won Deal</label><select id="prProspect"></select></div><div><label>Service Type</label><select id="prType"><option>Website</option><option>AI / Aira</option><option>Automation</option><option>Custom System</option></select></div><div><label>Project Name</label><input id="prName" placeholder="Auto dari prospect jika kosong"></div><div><label>Status</label><select id="prStatus"><option>Planning</option><option>Intake Pending</option><option>In Progress</option><option>Review</option><option>Completed</option><option>On Hold</option></select></div></div><div class="office-actions"><button class="btn gold" id="saveProject">Save Won Project</button></div></div><div class="editor-card"><h2>Won Project Register</h2><div class="table-wrap"><table class="office-table"><thead><tr><th>Project No</th><th>Project</th><th>Prospect</th><th>Package / Value</th><th>Status</th><th>Intake</th></tr></thead><tbody id="projectsTbody"></tbody></table></div></div><div class="editor-card"><h2>Project Intake Form</h2><p class="muted">Lengkapkan maklumat domain, hosting, scope, assets dan PM notes sebelum handover ke Office RH.</p><div class="office-form-grid"><div><label>Won Project</label><select id="intProject"></select></div><div><label>Company / Client</label><input id="intCompany" placeholder="Nama syarikat / client"></div><div><label>PIC</label><input id="intPic" placeholder="Nama PIC"></div><div><label>Phone</label><input id="intPhone" placeholder="Telefon"></div><div><label>Email</label><input id="intEmail" placeholder="Email"></div><div><label>Address</label><input id="intAddress" placeholder="Alamat / lokasi"></div></div><div class="office-form-grid three"><div><label>Ada Domain?</label><select id="intHasDomain"><option>Belum Semak</option><option>Ya</option><option>Tidak</option></select></div><div><label>Domain Name</label><input id="intDomain" placeholder="contoh: yourdomain.com"></div><div><label>Ada Hosting?</label><select id="intHasHosting"><option>Belum Semak</option><option>Ya</option><option>Tidak</option></select></div><div><label>Hosting Type</label><select id="intHostingType"><option>Belum Semak</option><option>cPanel</option><option>Cloudflare</option><option>Server Sendiri</option><option>RH Hosting</option><option>Lain-lain</option></select></div><div><label>Vendor Lama</label><input id="intVendor" placeholder="Jika ada"></div><div><label>Project Scope</label><select id="intScope"><option>Corporate Website</option><option>Service Website</option><option>Landing Page</option><option>Ecommerce</option><option>CRM</option><option>ERP</option><option>Booking System</option><option>Custom System</option></select></div></div><div class="office-form-grid"><div><label>Deliverables</label><div id="intDeliverables" class="check-grid">${['AI Chatbot','Lead System','Blog','Portfolio','Gallery','Google Map','WhatsApp','SEO Setup','Analytics','Dashboard','Admin Panel'].map(x=>`<label><input type="checkbox" value="${x}"> ${x}</label>`).join('')}</div></div><div><label>Assets Checklist</label><div id="intAssets" class="check-grid">${['Logo','Company Profile','Service List','Product List','Photos','Videos','Brand Guideline','Copywriting'].map(x=>`<label><input type="checkbox" value="${x}"> ${x}</label>`).join('')}</div></div></div><div class="office-form-grid"><div><label>Reference / Competitor Website</label><input id="intReference" placeholder="Link rujukan"></div><div><label>Special Requirement</label><input id="intSpecial" placeholder="Keperluan khas"></div></div><label>PM Notes</label><textarea id="intNotes" rows="4" placeholder="Catatan PM, risiko, info hosting/domain, bahan yang belum diterima..."></textarea><div class="office-actions"><button class="btn gold" id="saveIntake">Save Project Intake</button><span class="status-pill status-pending" id="intReadinessPreview">Readiness 0%</span></div></div><div class="editor-card"><h2>Intake Register</h2><div class="table-wrap"><table class="office-table"><thead><tr><th>Intake No</th><th>Project</th><th>Client</th><th>Scope</th><th>Readiness</th><th>Status</th><th>Action</th></tr></thead><tbody id="intakesTbody"></tbody></table></div></div>`}
  function documentsHTML(){return `<div class="section-head office-section"><div><div class="eyebrow">Filing System</div><h1>Filing</h1><p class="muted">Filing quotation, negotiation, won deal, project dan intake. Dokumen operasi penuh boleh dipindahkan ke Office RH.</p></div></div><div class="office-module-grid"><div class="module-card"><h3>Quotations</h3><p id="docQtCount">0 documents</p></div><div class="module-card"><h3>Negotiation</h3><p id="docInvCount">0 records</p></div><div class="module-card"><h3>Won Deals</h3><p id="docPayCount">0 records</p></div><div class="module-card"><h3>Project Intakes</h3><p id="docIntakeCount">0 intake files</p></div></div><div class="editor-card"><h2>Recommended Filing</h2><pre style="white-space:pre-wrap;background:#0f172a;color:#e5e7eb;border-radius:18px;padding:18px;overflow:auto">/clients/PRO-2026-0001/
  lead/lead.json
  prospect/profile.json
  quotation/QT-2026-0001.pdf
  negotiation/notes.json
  won-deal/WD-2026-0001.json
  project/PRJ-2026-0001/
    intake/INT-2026-0001.json
    assets/logo.png
    assets/photos/
    pm-notes.md</pre></div>`}
  ...
  function bindEvents(){
    document.addEventListener('click', e => {
      ...
      if(id==='refreshLeads') refreshOfficeLeads(); if(id==='addProspect') addProspect(); if(id==='addQtItem') addQtItem(); if(id==='saveQuotation') saveQuotation(); if(id==='previewQuotation') previewQuotation(); if(id==='newQuotationBtn') resetQuotation(); if(id==='savePayment') savePayment(); if(id==='saveProject') saveProject(); if(id==='newInvoiceBtn') createManualInvoice(); if(id==='saveIntake') saveIntake();
      ...
      const dealIdx=e.target.closest('[data-create-project-from-deal]')?.dataset.createProjectFromDeal; if(dealIdx!==undefined) createProjectFromDeal(Number(dealIdx)); const intakePr=e.target.closest('[data-create-intake-project]')?.dataset.createIntakeProject; if(intakePr!==undefined) createIntakeFromProject(Number(intakePr)); const officeInt=e.target.closest('[data-create-office-project]')?.dataset.createOfficeProject; if(officeInt!==undefined) sendIntakeToOffice(Number(officeInt)); const loadInt=e.target.closest('[data-load-intake]')?.dataset.loadIntake; if(loadInt!==undefined) loadIntakeToForm(Number(loadInt));
  ...
  function createProjectFromDeal(i){
    ...
    const pr={no:nextNo('PRJ',db.projects,'no'), dealNo:deal.no, customer:deal.prospect||'', name:`${deal.prospect||'Client'} - ${deal.package||'Website Project'}`, type:'Website', package:deal.package||'', value:deal.amount||0, status:'Intake Pending', intakeNo:'', created:today()};
    db.projects.unshift(pr); deal.createdProject=true; deal.projectNo=pr.no; save(); setTab('projects');
  }
  function checkedValues(id){ return Array.from(document.querySelectorAll(`#${id} input[type=checkbox]:checked`)).map(x=>x.value); }
  function readinessScore(intake){
    const checks=[intake.hasDomain==='Ya', !!intake.domainName, intake.hasHosting==='Ya', intake.hostingType && intake.hostingType!=='Belum Semak', !!intake.scope, (intake.deliverables||[]).length>0, (intake.assets||[]).includes('Logo'), (intake.assets||[]).some(x=>['Company Profile','Service List','Product List','Copywriting'].includes(x)), !!intake.pic, !!intake.phone];
    return Math.round((checks.filter(Boolean).length/checks.length)*100);
  }
  function currentProjectForIntake(){ const no=$('#intProject')?.value||''; return db.projects.find(p=>p.no===no) || null; }
  function saveIntake(){
    const pr=currentProjectForIntake(); if(!pr) return alert('Pilih Won Project dahulu.');
    const existing=db.intakes.find(x=>x.projectNo===pr.no) || null;
    const intake=existing || {no:nextNo('INT',db.intakes,'no'), projectNo:pr.no, dealNo:pr.dealNo||'', created:today(), status:'Intake Pending'};
    intake.client=$('#intCompany')?.value || pr.customer || '';
    intake.projectName=pr.name||''; intake.package=pr.package||''; intake.value=pr.value||0;
    intake.pic=$('#intPic')?.value||''; intake.phone=$('#intPhone')?.value||''; intake.email=$('#intEmail')?.value||''; intake.address=$('#intAddress')?.value||'';
    intake.hasDomain=$('#intHasDomain')?.value||'Belum Semak'; intake.domainName=$('#intDomain')?.value||''; intake.hasHosting=$('#intHasHosting')?.value||'Belum Semak'; intake.hostingType=$('#intHostingType')?.value||'Belum Semak'; intake.vendor=$('#intVendor')?.value||'';
    intake.scope=$('#intScope')?.value||'Corporate Website'; intake.deliverables=checkedValues('intDeliverables'); intake.assets=checkedValues('intAssets'); intake.reference=$('#intReference')?.value||''; intake.special=$('#intSpecial')?.value||''; intake.notes=$('#intNotes')?.value||'';
    intake.readiness=readinessScore(intake); intake.updated=today();
    if(!existing) db.intakes.unshift(intake);
    pr.intakeNo=intake.no; pr.status = intake.readiness>=80 ? 'Intake Ready' : 'Intake Pending';
    save(); alert(`Project Intake ${intake.no} disimpan. Readiness ${intake.readiness}%.`);
  }
  function createIntakeFromProject(i){ const pr=db.projects[i]; if(!pr) return; setTab('projects'); setTimeout(()=>{ const sel=$('#intProject'); if(sel) sel.value=pr.no; preloadIntakeForm(pr); },50); }
  function preloadIntakeForm(pr){
    const existing=db.intakes.find(x=>x.projectNo===pr.no);
    const deal=db.payments.find(p=>p.no===pr.dealNo) || {};
    const prospect=db.customers.find(c=>c.name===pr.customer) || {};
    if($('#intCompany')) $('#intCompany').value=existing?.client || pr.customer || '';
    if($('#intPic')) $('#intPic').value=existing?.pic || prospect.name || pr.customer || '';
    if($('#intPhone')) $('#intPhone').value=existing?.phone || prospect.phone || '';
    if($('#intEmail')) $('#intEmail').value=existing?.email || prospect.email || '';
    if($('#intAddress')) $('#intAddress').value=existing?.address || prospect.address || '';
    if($('#intHasDomain')) $('#intHasDomain').value=existing?.hasDomain || (prospect.domainStatus ? 'Ya' : 'Belum Semak');
    if($('#intDomain')) $('#intDomain').value=existing?.domainName || '';
    if($('#intHasHosting')) $('#intHasHosting').value=existing?.hasHosting || 'Belum Semak';
    if($('#intHostingType')) $('#intHostingType').value=existing?.hostingType || 'Belum Semak';
    if($('#intVendor')) $('#intVendor').value=existing?.vendor || '';
    if($('#intScope')) $('#intScope').value=existing?.scope || (String(pr.type||'').includes('Website')?'Corporate Website':'Custom System');
    if($('#intReference')) $('#intReference').value=existing?.reference || '';
    if($('#intSpecial')) $('#intSpecial').value=existing?.special || '';
    if($('#intNotes')) $('#intNotes').value=existing?.notes || `Package: ${pr.package||deal.package||'-'}\nValue: ${money(pr.value||deal.amount||0)}\nSource: Won Deal ${pr.dealNo||'-'}`;
    ['intDeliverables','intAssets'].forEach(id=>document.querySelectorAll(`#${id} input`).forEach(ch=>{ ch.checked=(existing?.[id==='intDeliverables'?'deliverables':'assets']||[]).includes(ch.value); }));
    if($('#intReadinessPreview')) $('#intReadinessPreview').textContent=`Readiness ${existing?.readiness||0}%`;
  }
  function loadIntakeToForm(i){ const intake=db.intakes[i]; if(!intake) return; const pr=db.projects.find(p=>p.no===intake.projectNo); if(pr){ const sel=$('#intProject'); if(sel) sel.value=pr.no; preloadIntakeForm(pr); } }
  function sendIntakeToOffice(i){ const intake=db.intakes[i]; if(!intake) return; intake.status='Office Queue'; intake.officeQueuedAt=today(); const pr=db.projects.find(p=>p.no===intake.projectNo); if(pr) pr.status='Office Queue'; save(); alert(`${intake.no} dihantar ke Office RH Queue.`); }
  ...
  function renderAll(){
    ...
    const pr=$('#projectsTbody'); if(pr) pr.innerHTML=db.projects.map((p,i)=>`<tr><td>${p.no}</td><td>${esc(p.name)}</td><td>${esc(p.customer)}</td><td>${esc(p.package||p.type||'-')}<br><span class="office-mini">${money(p.value||0)}</span></td><td>${pill(p.status)}</td><td>${p.intakeNo?`<span class="status-pill status-paid">${esc(p.intakeNo)}</span> <button class="btn ghost" data-create-intake-project="${i}">Edit</button>`:`<button class="btn gold" data-create-intake-project="${i}">Create Intake</button>`}</td></tr>`).join('') || '<tr><td colspan="6" class="muted">No project yet.</td></tr>';
    const intSel=$('#intProject'); if(intSel){ intSel.innerHTML=db.projects.map(p=>`<option value="${p.no}">${p.no} — ${esc(p.name)} — ${esc(p.customer)}</option>`).join('') || '<option value="">No project yet</option>'; }
    const itb=$('#intakesTbody'); if(itb) itb.innerHTML=(db.intakes||[]).map((x,i)=>`<tr><td><b>${esc(x.no)}</b><br><span class="office-mini">${esc(x.updated||x.created||'-')}</span></td><td>${esc(x.projectNo)}<br><span class="office-mini">${esc(x.projectName||'')}</span></td><td>${esc(x.client||'-')}<br><span class="office-mini">${esc(x.pic||'')}</span></td><td>${esc(x.scope||'-')}<br><span class="office-mini">${esc(x.domainName||'No domain')}</span></td><td><b>${x.readiness||0}%</b></td><td>${pill(x.status||'Intake Pending')}</td><td><button class="btn ghost" data-load-intake="${i}">Edit</button> <button class="btn gold" data-create-office-project="${i}">Create Office Project</button></td></tr>`).join('') || '<tr><td colspan="7" class="muted">No project intake yet.</td></tr>';
