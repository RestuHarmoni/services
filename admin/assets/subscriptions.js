...
const RHSubscriptions = (() => {
  ...
  function renderSubscriptionsTable(){
    ...
    body.innerHTML=rows.map(s=>{
      ...
      return `<tr><td data-label="No"><strong>${esc(s.subscription_no||'-')}</strong><span class="small-muted">${esc(s.project_no||'')}</span></td><td data-label="Client"><strong>${esc(s.client_name||'-')}</strong><span class="small-muted">${esc(s.phone||'')}</span></td><td data-label="Plan">${esc(s.plan_name||s.package_name||'-')}<span class="small-muted">${esc(s.package_name||'')}</span></td><td data-label="Billing"><strong>${fmtRM(s.cycle_amount||0)}</strong><span class="small-muted">${cycleLabel(s.billing_cycle)} • Discount ${fmtRM(s.discount_amount||0)}</span></td><td data-label="Next Billing">${dateShort(s.next_billing_date||s.current_period_end)}</td><td data-label="Status"><span class="status-pill ${st}">${statusLabel(st)}</span></td><td data-label="Action"><div class="table-actions"><button class="mini-btn primary" data-view-subscription="${s.id}">View</button><button class="mini-btn" data-edit-subscription="${s.id}">Edit</button><button class="mini-btn" data-renew-subscription="${s.id}">Renew Invoice</button><button class="mini-btn danger-action" data-delete-subscription="${s.id}">Delete</button></div></td></tr>`;
    }).join('');
  }
  function renderSubscriptionInvoicesTable(){
    ...
    body.innerHTML=rows.map(i=>{
      ...
      return `<tr><td data-label="Invoice"><strong>${esc(i.invoice_no||'-')}</strong><span class="small-muted">${esc(i.billing_cycle||'')}</span></td><td data-label="Client">${esc(i.client_name||'-')}</td><td data-label="Period">${dateShort(i.period_start)} → ${dateShort(i.period_end)}</td><td data-label="Amount"><strong>${fmtRM(i.amount||0)}</strong></td><td data-label="Due">${dateShort(i.due_date)}</td><td data-label="Status"><span class="status-pill ${st}">${statusLabel(st)}</span></td><td data-label="Action"><div class="table-actions">${st==='paid'?`<span class="small-muted">Paid ${dateShort(i.paid_at)}</span>`:`<button class="mini-btn primary" data-mark-sub-invoice-paid="${i.id}">Mark Paid</button>`}</div></td></tr>`;
    }).join('');
  }
