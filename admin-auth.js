(function(){
  ...
  async function getClient(){if(client)return client;if(window.RHGetSupabaseClient){client=await window.RHGetSupabaseClient({throwOnMissing:true});return client;}if(!cfg.url||!cfg.anonKey||cfg.url.includes('PASTE_'))throw new Error('Supabase config belum lengkap.');const ok=await loadSupabase();if(!ok||!window.supabase)throw new Error('Supabase library gagal dimuatkan.');client=window.supabase.createClient(cfg.url,cfg.anonKey);return client;}
