...
(function(){
  ...
  function buildAiraNotes(data){
    const lines=[];
    lines.push("=== AIRA LEAD INTAKE ===");
    lines.push("Jenis bisnes: "+(data.business_type||"-"));
    lines.push("Tujuan website: "+(data.objective||"-"));
    lines.push("Budget: "+(data.budget||"-"));
    lines.push("Timeline: "+(data.timeline||"-"));
    lines.push("Domain: "+(data.domain_status||"-"));
    lines.push("Logo: "+(data.logo_status||"-"));
    lines.push("Website lama: "+(data.existing_website||"-"));
    lines.push("Bilangan halaman: "+(data.pages_required||"-"));
    lines.push("Perlu WhatsApp: "+(data.whatsapp_required||"-"));
    lines.push("Perlu Booking/Form: "+(data.booking_required||"-"));
    lines.push("Perlu SEO Google: "+(data.seo_required||"-"));
    if(data.extra_notes)lines.push("Nota client: "+data.extra_notes);
    if(data.faq_topics)lines.push("FAQ ditanya: "+data.faq_topics);
    if(Array.isArray(data.aira_conversation)&&data.aira_conversation.length){
      lines.push("");
      lines.push("=== Q&A AIRA ===");
      data.aira_conversation.forEach((x,i)=>{lines.push((i+1)+". "+x.question+" => "+x.answer);});
    }
    return lines.join("\n");
  }
  function createLeadPayload(data){const recommended_package=packageFor(data);const lead_score=scoreLead(data);return{name:data.name,phone:normalizePhone(data.phone),business_type:data.business_type,objective:data.objective,budget:data.budget,timeline:data.timeline,recommended_package,lead_score,lead_temperature:tempFor(lead_score),status:"NEW",notes:buildAiraNotes(data),source:"Aira",page_url:location.href,user_agent:navigator.userAgent};}
  window.RH_AIRA_LEAD_SYSTEM={BANK,initSupabase,normalizePhone,validPhone,scoreLead,packageFor,priceFor,tempFor,featuresFor,getLocalLeads,saveLead,buildAiraNotes,createLeadPayload,matchFAQ};
})();
